...
11. **Proposition 13 (period lower bound from coverage) — RETRACTED.** The counting argument
    ...
    vacuous. The smallest witness is h=1, word "1": machine period 2, visits both classes;
    the nonconstant h=6, word "10" witness has machine period 8 and visits 7 classes.
...
theorem about Rule 30. The integer transition and observation helpers check
their finite-domain inputs at the API boundary: `horizon` is a non-negative
integer, `state` is width-limited, and each boundary bit is binary. This
validation also runs for empty boundary words. For
...
finite nested map between adjacent finite horizons. `PredictivePartition.same_horizon_transition_relation()`
exposes the explicitly set-valued finite relation that was intentionally not
implied by that nested map: each source class and boundary bit maps to every
same-horizon target class reached by its members. A target set may contain
multiple classes, so this is not a deterministic automaton or an
...
| `sibling_fiber_parity.py` | Exact raw finite ρ/τ sibling audit | h≤13 empirically checks the parity pattern; h=1 is a degenerate exception |
