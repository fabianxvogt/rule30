...
Added `experiments/sibling_fiber_parity.py`, a dependency-free exact check of
the finite predictive partitions `S_h` through a hard-coded maximum horizon of
`h = 10`. It computes the right-truncation map `rho_h`, the two nested child
...
For every checked `2 <= h <= 10`, all `rho_h` fibers are singletons or
doubletons, and every doubleton sibling pair has the same leading bit. For
every checked `3 <= h <= 10`:
...
in every partition `S_h` for `0 <= h <= 10`, checks both boundary bits, and
...
This does not prove the parity pattern for `h > 10`, establish an
...
```text
python3 experiments/sibling_fiber_parity.py --max-horizon 10
...
boundary correction to the older observation wording. No infinite result or
